...
import "./interfaces/IPoqLotteryV3.sol";

/** @title Poq Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract PoqLotteryV3 is ReentrancyGuard, IPoqLotteryV3, Ownable {
    ...
    uint256 public maxPriceTicketInPoq = 50 ether;
    uint256 public minPriceTicketInPoq = 0.005 ether;
    ...
    IPoqDrandGeneratorV2 public randomGenerator;
     ...
     */
    constructor(address _randomGeneratorAddress) Ownable(msg.sender) {
        randomGenerator = IPoqDrandGeneratorV2(_randomGeneratorAddress);
        ...
        nonReentrant
    {
		...
		uint256 amountPointToTransfer = _calculateTotalPriceForBulkTickets(
			isPoint?  priceTicketInPoint : _lotteries[_lotteryId].priceTicketInPoq,
		...
		if (isPoint) {
			...
			require( amountPointToTransfer <= msg.value ,"Insufficient native token");

			// Increment the total amount collected for the lottery round
			_lotteries[_lotteryId].amountCollectedInPoq += amountPointToTransfer;

			// refund
			if( msg.value > amountPointToTransfer ){
				(bool ok,) = payable(msg.sender).call{value: msg.value - amountPointToTransfer}("");
				require(ok, "refund fail");
			}
		}
    ...
    function claimTickets(
		...
		(bool ok,) = payable(msg.sender).call{value: rewardInCakeToTransfer}("");
        ...
        nonReentrant
    {
		...
		(bool ok,) = payable(treasuryAddress).call{value: amountToWithdrawToTreasury}("");
        require(ok, "Transfer to treasury address failed");
    ...
    function injectFunds(uint256 _lotteryId) external payable override onlyOwnerOrInjector {
        ...
        _lotteries[_lotteryId].amountCollectedInPoq += msg.value;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInPoq >= minPriceTicketInPoq) && (_priceTicketInPoq <= maxPriceTicketInPoq),
            "Outside of limits"
        );
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(0x0), "Cannot be Poq token");
        ...
        onlyOwner
    {
        ...
        minPriceTicketInPoq = _minPriceTicketInCake;
        maxPriceTicketInPoq = _maxPriceTicketInCake;
    }
